...
function MzTreeView(Tname)
{
  ...
  this.toString = function()
  {
    ...
    var str = "<A id='"+ this.name +"_RootLink'  style='DISPLAY: none'></A>";
    ...
    if(rootCN.length>0)
    {
      ...
      //  this.name +".focusClientNode('"+ rootCN[0].id +"'); "+ this.name +".atRootIsEmpty()",10);//xh
      setTimeout(this.name +".focusClientNode('"+ rootCN[0].id +"'); "+ this.name +".atRootIsEmpty()",10);
    }
...
MzTreeView.prototype.atRootIsEmpty = function()
{
  ...
  {
    //if(!RCN[i].isLoad) this.expand(RCN[i].id);//xh
    if (RCN[i].T=="")
    {
...
MzTreeView.prototype.focusClientNode      = function(id)
{
 try {
  ...
  //backgroundColor = this.colors.highLight;}//xh
