...
<html>
	...
	<script>
		...
		function save() {
			...
			target.action = "${ctx}/order/orderInfo.do?method=saveSp&tempFlag=true";
		...
		function sendOrder() {
			...
			var target = document.getElementById("orderForm");
			target.action = "${ctx}/order/orderInfo.do?method=saveSp";
			document.getElementById("itemInput").value=data;
			target.submit();
		}
		...
		//创建物料选择树型输入框
		//function CreateProdBox(element){
		//	var editState = element.getAttribute("EditState");
		//	if(editState!="true"){
		//		var textBox = document.createElement("INPUT");
		//		textBox.type = "text";
		//		textBox.style.className = "select_but";
		...
		function CreateTextBox(element){
			...
			if(editState!="true"){
				...
				//numberMultiply(element.parentNode.cells[5].innerHTML,element.parentNode.cells[4].innerHTML,element.parentNode.cells[6].innerHTML);
	   ...
	   function numberMultiply(v1, v2,obj){
		var jsonUrl = "${ctx}/order/orderInfo.do?method=numberMultiply&scale=3&v1="+v1+"&v2"+v2;
			$.ajax({
			   type: "POST",
			   url: jsonUrl,
			   async: false,
	...
	<body onLoad="WindowSollAuto();onLoad1()" onResize="WindowSollAuto()">
		...
		<html:form styleId="orderForm" action="/order/orderInfo.do?method=saveSp" method="post">
			...
			<input type="hidden" name="itemBrandId" id="itemBrandId" value="${itemBrandId}"/>
			<div class="bosom_one">
				<div class="bosom_top">
							...
							<div class="sheet_div" style="width:100%;">
								<table class="list_add1" id="orderTable" style="display:block">
								    ...
								    <tr>
										...
										<td align="left">
										   <input type="text" name="queryDate" id="queryDate" onfocus="setDay(this)" readonly value="${shiptoDate }" class="date_but" onchange="getDeliveryDays()"/>
										</td>
										
									</tr>
								    <tr>
										<td class="formTable" width="4%">客户编码</td>
										<td align="left" width="8%">${dist.distCode}
										    
										 </td>
										<td class="formTable" width="4%">客户名称</td>
										<td align="left" width="8%">${dist.distName}
									...
									<tr>
										...
										<td align="left" id="linkAddrTD"> ${address.shiptoAdd }
										</td>
										...
										<td align="left" width="8%"> 
											<select id="modeOfTransportType" name="modeOfTransportType"style='width:155px;' >
												...
												<c:forEach items="${modeOfTransportTypes}" var="modeType">
													<option value="${modeType.dictItemId }">${modeType.itemName }</option>
												</c:forEach>
											</select>
										</td> 
									</tr>
								...
								<!-- <div style="width:100%;text-align:right;padding-right:50px;padding-top:10px;padding-bottom:10px;">
									...
									<input type="button" style="width:50px;border:1px solid #99CCFF" onclick="javascript:save();" value="草稿"/>
									<input type="button" style="width:50px;border:1px solid #99CCFF" onclick="javascript:sendOrder();" value="提交"/>
								</div> -->
							</div>
						</div>
						<div class="trail">
