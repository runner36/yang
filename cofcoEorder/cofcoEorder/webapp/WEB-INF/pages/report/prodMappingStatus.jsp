...
<script>
			...
			$(m).find("td").each(function(index,object){
				...
				if (index == prodNameIndex) { //1
					...
					if($(object).text().trim()!='' && $(object).text().trim()!=' ' &&$(object).text() != '&#160;'){
						...
						codeTd.css('text-decoration','underline').css('cursor','pointer').click(function(){
							openWindow("${ctx}/server/dmsProdMapping.do?method=list&dist_code="+distCode+"&distProdCode_="+prodCode,"distProdName_",prodName);
						});
					}
				}
				...
				if(index==addAmountRateIndex){ //加价率 10
					if($(object).text().trim()!='' && $(object).text().trim()!=' ' &&$(object).text() != '&#160;'){
						codeTd = $(object);
						codeTd.css('text-decoration','underline').css('cursor','pointer').click(function(){
							openWindow("${ctx}/report/listReport.do?reportName=server/checkSaleList&distCode="+distCode+"&billDate1="+startDate+"&billDate2="+monthLastDay+"&state=1&prodCode="+prodCode+"&ec_crd=1000&colIds=","prodName",prodName);
						});
					}
				}
				...
				if(index==arrivalQtyRateIndex){ //到货比率 14
					if($(object).text().trim()!='' && $(object).text().trim()!=' ' &&$(object).text() != '&#160;'){
						codeTd = $(object);
						codeTd.css('text-decoration','underline').css('cursor','pointer').click(function(){
							openWindow("${ctx}/report/listReport.do?reportName=prodMappingStatusArrivalRate&distCode="+distCode+"&startDate="+startDate+"&endDate="+monthLastDay+"&prodCode="+prodCode+"&ec_crd=1000&colIds=","prodName",prodName);
						});
					}
				}
			});
	...
	$(document).ready(function(){
		...
		var startDate="${date1}-01";
		var endDate	= new Date("${date2}".replace("-", "/")+"/01" ); 
		...
		<tr>
			...
			<td >
				<html:text styleId="start_date" property="start_date" value="${date1}" onfocus="WdatePicker({dateFmt:'yyyy-MM'})" readonly="true" styleClass="date_but"/>~
				<html:text styleId="end_date" property="end_date" value="${date2}" onfocus="WdatePicker({dateFmt:'yyyy-MM'})" readonly="true" styleClass="date_but"/>
			</td>
			<td width="1%">产品状态：</td>
			<td >
