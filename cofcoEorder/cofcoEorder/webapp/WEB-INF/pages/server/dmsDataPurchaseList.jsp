...
<html>
	<head>
		...
		<title>${mr['page.customize.title.dmsDataPurchase']}</title>
	    ...
	    function query() {
			resetec();
			var form = document.dmsDataPurchaseForm;
			//reg = /^[-\+]?\d+(\.\d+)?$/;
	...
	<body onload="WindowSollAuto();disabledInit();"><jsp:include page="/commons/messages.jsp" flush="true"></jsp:include>
		<html:form action="/server/dmsDataPurchase.do?method=list" method="post">
			<div class="bosom_one">
				<div class="bosom_top">
					<span class="left"></span>
					<h4>
						<!--TitleStrat-->
						${mr['page.customize.title.dmsDataPurchase']}
						<!--TitleStrat-->
					</h4>
					<div class='MenuList'>
						<!--MenuStrat-->
						...
						<a href="javascript:dmsDataPurchaseForm.ec_p.value=1;query()" id="query_A">${mr['page.common.button.query']}</a>
						...
						<div class="viscera" id="SollAuto">
							<div class="sheet_div">
								...
								<ec:table items="list" var="item" onInvokeAction="query()"
										form="dmsDataPurchaseForm" retrieveRowsCallback="limit"
										style="width:100%" tableId="ec"
										action="dmsDataPurchase.do?method=list">
									<ec:exportXls fileName="${mr['page.customize.title.dmsDataPurchase']}.xls" tooltip="${mr['page.common.exportExcel']}" view="xls" />
									<ec:exportCsv fileName="${mr['page.customize.title.dmsDataPurchase']}.csv" tooltip="${mr['page.common.exportCVS']}" view="csv" />
									<ec:row highlightRow="true">
										...
										<ec:column property="supplierCode" title="${mr['page.common.suppliercode']}" />
										<ec:column property="supplierName" title="${mr['page.common.supplierName']}"/>
										...
										<ec:column property="11" title="经销商金额(计算)"  format="0.000" style="text-align:right">											
											<fmt:formatNumber value="${item.prodPrice*item.prodQuantity}" pattern="#00.000#"/> 
										</ec:column>
										<ec:column property="12" title="计算差额"  style="text-align:right">											
											<fmt:formatNumber value="${item.prodPrice*item.prodQuantity-item.prodAmount}" pattern="#00.000#"/> 
										</ec:column>
